...
let parse_toml filename : config option =
  ...
  | `Yes ->
      let toml =
        ...
        | `Error _ ->
            failwith "could not parse fromage.toml"
        | `Ok toml ->
            toml
      in
      let config =
      ...
      let ignored_files =
        ...
        with
        | Some (TArray (NodeString ignored_files)) ->
            ignored_files
        | _ ->
            []
      in
      let ignored_dirs =
        ...
        with
        | Some (TArray (NodeString ignored_dirs)) ->
            ignored_dirs
        | _ ->
            []
      in
...
let is_ignored_dir (path : string) ~config =
  ...
  then true
    ...
    | None ->
        false
    | Some config ->
        List.mem ~equal:String.( = ) config.ignored_dirs path
...
let is_ignored_file path ~config =
  ...
  if Char.(first_letter = '.') then true
    ...
    | None ->
        false
    | Some config ->
        List.mem ~equal:String.( = ) config.ignored_files path
